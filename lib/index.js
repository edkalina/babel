...
You can turn on the 'throwIfNamespace' flag to bypass this warning.`);
  ...
  function buildElementCall(path, file) {
    ...
    const [initialProps, decorators] = splitDecorators(openingPath.node.attributes);
    decorators.push(state.callee);
    args.push(buildOpeningElementAttributes(initialProps, file), ...path.node.children);
    ...
    return decorators.reduce((expr, decorator) => {
      return t().callExpression(t().memberExpression(decorator, t().identifier("apply")), [t().nullLiteral(), expr]);
    }, initialExpr);
  }
  ...
  function splitDecorators(attribs) {
    ...
    for (let i = 0; i < attribs.length; i++) {
      ...
      if (t().isJSXAttribute(attr) && attr.isMeta) {
        ...
        decorators.push(t().callExpression(name, value ? [convertAttributeValue(value)] : []));
      } else {
